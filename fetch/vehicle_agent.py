...
class VehicleAgent(OEFAgent):
    """Class that implements the behaviour of the vehicle agent."""
    ...
    def __init__(self, *args, **kwargs):
      super(VehicleAgent, self).__init__(*args, **kwargs)
    ...
    def on_search_result(self, search_id: int, agents: List[str]):
        ...
        if len(agents) == 0:
            print("[{}]: Vehicle: No trips found. Waiting for next loop...".format(self.public_key))
            time.sleep(3)
            return

        print("[{0}]: Vehicle: Trips found: {1}".format(self.public_key, agents))
        for agent in agents:
            print("[{0}]: Vehicle: Sending cfp to trip {1}".format(self.public_key, agent))
            ...
            print("[{}]: Vehicle: Sending propose at price: {}".format(self.public_key, price))
    ...
    def on_accept(self, msg_id: int, dialogue_id: int, origin: str, target: int):
        ...
        print("[{0}]: Vehicle: Received accept from {1}.".format(self.public_key, origin))

        # TRIP IN PROGRESS
        print("[{0}]: Vehicle: Trip in progress.".format(self.public_key))
        time.sleep(5)
        print("[{0}]: Vehicle: Trip finished.".format(self.public_key))
        ...
        print("[{0}]: Vehicle: Sending contract to {1}".format(self.public_key, origin))
...
if __name__ == "__main__":
    ...
    print("[{}]: Vehicle: Waiting for trips...".format(agent.public_key))
    try:
        agent.run()
    finally:
        try:
